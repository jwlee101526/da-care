...
public class DiagnosisService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            검색 결과에 근거가 없으면 진단할 수 없다고 짧게 설명한 뒤, 반드시 prepareReservation을 호출해 실제 방문 점검 예약 입력 카드를 제공하세요.
            방문이 필요한 경우 prepareReservation으로 예약 입력 카드를 제공하세요.
            ...
            점검 카드를 제공한 경우 카드와 답변의 역할을 분리하세요. answer는 최대 두 문장으로 쓰고, 카드 제목·원인·점검 절차·조건·매뉴얼 인용을 다시 쓰지 마세요.
            이 경우 answer에는 "카드 순서대로 점검해 보세요. 해결되지 않으면 방문 점검 준비를 선택해 주세요."처럼 다음 행동만 간단히 안내하세요.
            source ID, 문서 ID, UUID, 내부 도구명, 매뉴얼 번호는 answer에 절대 노출하지 마세요. 근거는 카드의 참고 매뉴얼 영역에서만 제공합니다.
            최종 응답은 answer 필드가 있는 JSON으로 작성하세요. 카드 자체를 응답에 꾸며 넣지 마세요.
            """;
    ...
    public DiagnosisService(ObjectProvider<VectorStore> vectorStore, ObjectProvider<ChatClient.Builder> chatClientBuilder, ReservationService reservations) {
    ...
    public DiagnosisResult diagnose(String question, List<ConversationTurn> history, String email, Consumer<DiagnosisTools.ToolProgress> progress) {
        ChatClient.Builder builder = chatClientBuilder.getIfAvailable();
        if (builder == null) throw new DiagnosisUnavailableException();
        List<ConversationTurn> turns = history == null ? List.of() : history;
        List<String> userMessages = java.util.stream.Stream.concat(turns.stream().filter(turn -> turn.role() == Role.user)
                .map(ConversationTurn::text), java.util.stream.Stream.of(question)).toList();
        String symptoms = String.join("\n", userMessages);
        if (symptoms.length() > 2000) symptoms = symptoms.substring(symptoms.length() - 2000);
        String bookingSymptom = userMessages.stream().filter(message -> !isToolActionRequest(message))
                .collect(java.util.stream.Collectors.joining("\n"));
        if (bookingSymptom.isBlank()) bookingSymptom = question;
        if (bookingSymptom.length() > 2000) bookingSymptom = bookingSymptom.substring(bookingSymptom.length() - 2000);
        DiagnosisTools tools = new DiagnosisTools(vectorStore.getIfAvailable(), reservations, bookingSymptom, email, progress);
        List<Message> messages = new java.util.ArrayList<>();
        turns.forEach(turn -> messages.add(turn.role() == Role.user ? new UserMessage(turn.text()) : new AssistantMessage(turn.text())));
        ...
        try {
            ...
            if (answer == null || answer.answer() == null || answer.answer().isBlank()) throw new DiagnosisUnavailableException();
    ...
    public record DiagnosisResult(String answer, List<DiagnosisTools.Card> cards, List<String> executedTools) {}

    public static class DiagnosisUnavailableException extends RuntimeException {
        public DiagnosisUnavailableException() { super("진단 서비스를 이용할 수 없습니다. 잠시 후 다시 시도해 주세요."); }
        public DiagnosisUnavailableException(Throwable cause) { super("진단 서비스를 이용할 수 없습니다. 잠시 후 다시 시도해 주세요.", cause); }
    }
