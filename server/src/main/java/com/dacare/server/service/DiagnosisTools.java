...
public class DiagnosisTools {
    ...
    public DiagnosisTools(VectorStore vectorStore, ReservationService reservations, String question, String email) {
        this(vectorStore, reservations, question, email, ignored -> {});
    }

    public DiagnosisTools(VectorStore vectorStore, ReservationService reservations, String question, String email, Consumer<ToolProgress> progress) {
    ...
    public synchronized List<ManualSource> searchManuals(String query) {
        ...
        if (vectorStore == null) throw new IllegalStateException("매뉴얼 검색 서비스를 사용할 수 없습니다.");
        var documents = vectorStore.similaritySearch(SearchRequest.builder().query(query).topK(3).similarityThreshold(0.65).build());
    ...
    public synchronized InspectionCard showInspectionCard(
        ...
        if (suspectedCause != null && !suspectedCause.isBlank()) requireText(suspectedCause, 500);
        requireText(inspectionDetails, 1000);
        if (sourceIds == null || sourceIds.isEmpty() || sourceIds.size() > 3) throw new IllegalArgumentException("매뉴얼 근거가 필요합니다.");
        List<Evidence> evidence = sourceIds.stream().distinct().map(id -> {
            ManualSource source = sources.get(id);
            if (source == null) throw new IllegalArgumentException("검색된 매뉴얼과 일치하지 않는 문서 ID입니다.");
            return new Evidence(source.id(), source.text());
        }).toList();
        String cause = suspectedCause == null || suspectedCause.isBlank() || sources.containsKey(suspectedCause) ? null : suspectedCause;
        InspectionCard card = new InspectionCard("inspection", title, deviceType, deviceName, cause, inspectionDetails, List.copyOf(evidence));
    ...
    public synchronized ReservationStatusCard getReservationStatus(long reservationId) {
        ...
        countCall();
        if (email == null) throw new IllegalStateException("예약 조회는 로그인이 필요합니다.");
        var reservation = reservations.mineOne(email, reservationId);
        ReservationStatusCard card = new ReservationStatusCard("reservation_status", reservation.getId(),
    ...
    public synchronized NavigationCard navigateTo(Page page) {
        ...
        NavigationCard card = switch (page) {
            case reservations -> new NavigationCard("navigation", page, "예약 내역 조회", "접수한 방문 점검의 상태와 확정 일정을 확인할 수 있습니다.", "예약 내역으로 이동");
        };
    ...
    private void countCall() {
        if (++calls > 8) throw new IllegalStateException("도구 호출 횟수를 초과했습니다.");
    }

    private void started(String tool) { progress.accept(new ToolProgress(tool, "started")); }
    private void completed(String tool) { progress.accept(new ToolProgress(tool, "completed")); }
    private void completed(String tool, Card card) { progress.accept(new ToolProgress(tool, "completed", card)); }

    private void requireText(String text, int maxLength) {
        if (text == null || text.isBlank() || text.length() > maxLength) throw new IllegalArgumentException("도구 입력값을 확인하세요.");
    }

    public synchronized List<Card> cards() { return List.copyOf(cards.values()); }
    public synchronized List<String> executedTools() { return List.copyOf(executedTools); }
    public enum DeviceType { laptop, smartphone, appliance, etc }
    public enum Page { reservations }
    public sealed interface Card permits InspectionCard, BookingCard, ReservationStatusCard, NavigationCard {}
    public record ManualSource(String id, String text) {}
    public record Evidence(String sourceId, String quote) {}
    public record InspectionCard(String type, String title, DeviceType deviceType, String deviceName, String suspectedCause,
                                 String inspectionDetails, List<Evidence> evidence) implements Card {}
    public record BookingCard(String type, DeviceType deviceType, String symptom, boolean loginRequired) implements Card {}
    public record ReservationStatusCard(String type, Long reservationId, String status, LocalDateTime preferredAt,
                                        LocalDateTime confirmedAt, String engineerName) implements Card {}
    public record NavigationCard(String type, Page page, String title, String description, String actionLabel) implements Card {}
    public record ToolProgress(String tool, String status, Card card) {
        public ToolProgress(String tool, String status) { this(tool, status, null); }
    }
}
