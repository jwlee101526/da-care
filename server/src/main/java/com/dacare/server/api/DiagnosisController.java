...
public class DiagnosisController {
    ...
    SseEmitter stream(@Valid @RequestBody QuestionRequest request, Authentication authentication) {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                DiagnosisService.DiagnosisResult result = service.diagnose(request.question(), request.history() == null ? List.of() : request.history(),
                        authentication == null ? null : authentication.getName(), progress -> send(emitter, "tool", progress));
