...
public class ApiExceptionHandler {
  ...
  @ExceptionHandler(com.dacare.server.service.DiagnosisService.DiagnosisUnavailableException.class)
  ResponseEntity<Map<String, Object>> diagnosisUnavailable(RuntimeException e) {
    return ResponseEntity.status(HttpStatus.SERVICE_UNAVAILABLE)
        .body(Map.of("code", "DIAGNOSIS_UNAVAILABLE", "message", e.getMessage()));
  }
  ...
  ResponseEntity<Map<String, Object>> validation(MethodArgumentNotValidException e) {
    ...
    return ResponseEntity.badRequest()
        .body(Map.of("code", "VALIDATION_ERROR", "message", "입력값을 확인하세요.", "fields", fields));
  }
}
