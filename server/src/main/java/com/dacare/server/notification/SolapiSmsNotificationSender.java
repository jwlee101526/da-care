...
public class SolapiSmsNotificationSender {
    ...
    public void sendReservationConfirmed(Reservation reservation) {
        ...
        if (!isConfigured()) {
            histories.save(new NotificationHistory(reservation, "SMS", "SKIPPED", message, "SOLAPI 설정 미완료"));
            return;
        }
        try {
            ...
            histories.save(new NotificationHistory(reservation, "SMS", "FAILED", message, e.getMessage()));
        }
    }
