...
public class SlackNotificationSender {
    ...
    public void sendReservationReceived(Reservation reservation) { String message = "DA-Care 새 예약 접수 #" + reservation.getId() + " / " + reservation.getDeviceType() + " / 희망 방문: " + reservation.getPreferredAt(); if (webhookUrl.isBlank()) { histories.save(new NotificationHistory(reservation, "SLACK", "SKIPPED", message, "SLACK_WEBHOOK_URL 미설정")); return; } try { client.post().uri(webhookUrl).body(Map.of("text", message)).retrieve().toBodilessEntity(); histories.save(new NotificationHistory(reservation, "SLACK", "SUCCESS", message, null)); } catch (RuntimeException e) { histories.save(new NotificationHistory(reservation, "SLACK", "FAILED", message, e.getMessage())); } }
}
